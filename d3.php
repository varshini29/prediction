...
function getForecast() {
    $times = array();
    global $conn;
    $sql = "SELECT * FROM rainfall WHERE status = 'active' AND drain_id = 3";
...
function calculatePeakDischarge($rainfall) {
    ...
    // ans :Q m3/s
    //y = x * 35.315 cfs
    $peak = $rainfall * 0.278 * 0.7 * 0.0975;
    $result = $peak * 35.315;
...
foreach($datasets as $value) {
   // print_r($value->precipitation);
    ...
    $document->addData(['forecasted_rainfall_intensiy' => $value->precipitation, 'depth_of_drain' => 4.79, 'peak_discharge' => $peak_discharge, 'drainage_capacity' => 186.14]);
...
$writer->write($document, 'C:\Users\varsh\Documents\Applied-Computing\Dissertation\WEKA\ANN\forecast_d3.arff');
